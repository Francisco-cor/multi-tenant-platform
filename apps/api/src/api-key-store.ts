...
const VALID_SCOPES = new Set(['webhooks:read','webhooks:manage','orders:read','orders:create','inventory:read','files:read','files:upload','audit:read','automations:read','automations:manage']);
...
export interface ApiKeyStore {
  create(context: StoreTenantContext, input: CreateApiKeyInput): Promise<{ record: ApiKeyRecord; raw: string }>;
...
export class InMemoryApiKeyStore implements ApiKeyStore {
  ...
  async create(context: StoreTenantContext, input: CreateApiKeyInput): Promise<{ record: ApiKeyRecord; raw: string }> {
  ...
  async verify(prefix: string, raw: string): Promise<ApiKeyRecord | null> {
    ...
    for (const k of this.keys.values()) {
      if (k.prefix===prefix && k.hash===hash && !k.revokedAt && (!k.expiresAt || k.expiresAt > Date.now())) return k;
    }
    return null;
  }

  getRaw(id:string): string | undefined { return this.rawById.get(id); }
}

export class PersistentApiKeyStore implements ApiKeyStore {
  constructor(private readonly db: DatabaseHandle) { if (!db.role) throw new Error('database_role_required');}
  static fromConnectionString(cs:string, role='platform_app'): PersistentApiKeyStore { return new PersistentApiKeyStore(createDatabase(cs,{role}));}
  async create(context: StoreTenantContext, input: CreateApiKeyInput): Promise<{ record: ApiKeyRecord; raw: string }> {
    validateScopes(input.scopes);
    const { raw, prefix, hash } = generateApiKey();
    const scopesJson = JSON.stringify(input.scopes);
    const expiresAt = input.expiresInMs ? new Date(Date.now()+input.expiresInMs).toISOString() : null;
    return withTenantTransaction(this.db, context, async (tx)=>{
      const rows = await tx.execute<{ id:string; tenant_id:string; prefix:string; hash:string; scopes:string; name:string; expires_at:string|null; revoked_at:string|null; created_by:string; created_at:string }>(sql`
      ...
      `);
      const r = rows[0]; if (!r) throw new Error('api_key_create_failed');
      const rec: ApiKeyRecord = {
        id:r.id, tenantId:r.tenant_id, prefix:r.prefix, hash:r.hash,
        scopes: JSON.parse(r.scopes), name:r.name,
  ...
  async list(context: StoreTenantContext): Promise<ApiKeyRecord[]> {
    return withTenantTransaction(this.db, context, async (tx)=>{
      ...
      `);
      return rows.map(r=>({ id:r.id, tenantId:r.tenant_id, prefix:r.prefix, hash:r.hash, scopes: JSON.parse(r.scopes), name:r.name, expiresAt: r.expires_at? new Date(r.expires_at).getTime():null, revokedAt: r.revoked_at? new Date(r.revoked_at).getTime():null, createdBy:r.created_by, createdAt: new Date(r.created_at).getTime()}));
    });
  }
  async revoke(context: StoreTenantContext, id:string): Promise<void> {
    await withTenantTransaction(this.db, context, async (tx)=>{
      const rows = await tx.execute<{ id:string }>(sql`update api_keys set revoked_at=now(), updated_at=now() where id=${id}::uuid and tenant_id=${context.tenantId}::uuid and revoked_at is null returning id`);
  ...
  async verify(prefix:string, raw:string): Promise<ApiKeyRecord | null> {
    ...
    const rows = await this.db.db.execute<{ id:string; tenant_id:string; prefix:string; hash:string; scopes:string; name:string; expires_at:string|null; revoked_at:string|null; created_by:string; created_at:string }>(sql`
    ...
    return { id:r.id, tenantId:r.tenant_id, prefix:r.prefix, hash:r.hash, scopes: JSON.parse(r.scopes), name:r.name, expiresAt: r.expires_at? new Date(r.expires_at).getTime():null, revokedAt: r.revoked_at? new Date(r.revoked_at).getTime():null, createdBy:r.created_by, createdAt: new Date(r.created_at).getTime()};
  }
}
