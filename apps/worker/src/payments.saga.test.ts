...
describe('payments saga anti-double-charge', () => {
  it('FakeProvider is idempotent via providerKey: same key returns same providerRef without double charge', async () => {
    ...
    const r1 = await fake.charge({ amountCents: 1000, currency: 'USD', idempotencyKey: key, orderId: 'order-1', tenantId: 'tenant-1' });
    const r2 = await fake.charge({ amountCents: 1000, currency: 'USD', idempotencyKey: key, orderId: 'order-1', tenantId: 'tenant-1' });
  ...
  it('kill after charge does not double charge: retry with same key returns 1 charge', async () => {
    ...
    try {
      await fake.charge({ amountCents: 2500, currency: 'USD', idempotencyKey: key, orderId: 'order-kill', tenantId: 'tenant-a' });
    } catch (e) {
      firstError = e;
    }
    ...
    const retry = await fake.charge({ amountCents: 2500, currency: 'USD', idempotencyKey: key, orderId: 'order-kill', tenantId: 'tenant-a' });
  ...
  it('different tenants/amounts produce different providerRefs (isolation)', async () => {
    const fake = new FakePaymentProvider({ mode: 'always_paid' });
    const r1 = await fake.charge({ amountCents: 100, currency: 'USD', idempotencyKey: 'key-tenant-a-amount-100', orderId: 'o1', tenantId: 'tenant-a' });
    const r2 = await fake.charge({ amountCents: 100, currency: 'USD', idempotencyKey: 'key-tenant-b-amount-100', orderId: 'o1', tenantId: 'tenant-b' });
    const r3 = await fake.charge({ amountCents: 200, currency: 'USD', idempotencyKey: 'key-tenant-a-amount-200', orderId: 'o1', tenantId: 'tenant-a' });
  ...
  it('unknown -> reconciler getStatus can resolve to paid', async () => {
    ...
    const charged = await fake.charge({ amountCents: 500, currency: 'USD', idempotencyKey: key, orderId: 'o-unknown', tenantId: 't1' });
