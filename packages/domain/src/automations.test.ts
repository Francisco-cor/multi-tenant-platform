...
describe('automations versioned commands', () => {
  ...
  it('rejects invalid trigger or unsupported version', () => {
    expect(() => validateAutomation({ trigger: 'invalid', action: { type: 'log' } })).toThrow(
      /automation_trigger_invalid/,
    );
    expect(() =>
      validateAutomation({ trigger: 'order.paid', action: { type: 'log' }, version: 99 }),
    ).toThrow(/automation_version_unsupported/);
  });
  ...
  it('rejects arbitrary code — only versioned types allowed', () => {
    expect(() =>
      validateAutomation({
        trigger: 'order.paid',
        action: { type: 'eval', code: 'rm -rf' },
      } as unknown as { trigger: string; action: unknown }),
    ).toThrow(/automation_action_type_invalid/);
    expect(() =>
      validateAutomation({
        trigger: 'order.paid',
        action: { type: 'webhook', params: 'not-object' } as unknown as {
          type: string;
          params: unknown;
        },
      }),
    ).toThrow(/automation_params_invalid/);
  });
